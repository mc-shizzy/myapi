...
/** Base URL for /api/download and /api/subtitles proxy (VPS / Heroku). */
function getProxyOrigin(req) {
  const fromEnv = process.env.STREAM_PROXY_URL || process.env.PROXY_ORIGIN;
  if (fromEnv) return String(fromEnv).replace(/\/$/, "");
  return getWorkerOrigin(req);
}
...
async function handleSources(req, res) {
  ...
  const workerOrigin = getProxyOrigin(req);
...
app.listen(PORT, () => {
  const proxy = process.env.STREAM_PROXY_URL || process.env.PROXY_ORIGIN || "(same host — use proxy-server.js)";
  console.log(`MovieBox API running on http://localhost:${PORT}`);
  console.log(`Stream proxy URL: ${proxy}`);
});
