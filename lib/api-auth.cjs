...
/** Encoded CDN path from /api/download/* or /api/subtitles/* (works before route params exist). */
function getStreamEncodedPathFromRequest(req) {
  const path = req.path || "";
  if (path.startsWith("/api/download/")) {
    return path.slice("/api/download/".length);
  }
  if (path.startsWith("/api/subtitles/")) {
    return path.slice("/api/subtitles/".length);
  }
  return "";
}
...
function requireApiKeyExpressMiddleware(corsHeaders) {
  return (req, res, next) => {
    ...
    if (isStream) {
      const encoded = getStreamEncodedPathFromRequest(req) || req.params[0] || "";
...
module.exports = {
  ...
  getStreamEncodedPathFromRequest,
