...
/** Search via VPS proxy (apii) — avoids MovieBox 429 on Cloudflare Worker IPs. */
async function searchViaProxy(proxyOrigin, keyword, page, perPage, subjectType) {
  const base = String(proxyOrigin).replace(/\/$/, "");
  const url = new URL(`${base}/api/search/${encodeURIComponent(keyword)}`);
  ...
  try {
    ...
    throw new Error(`Search proxy HTTP ${response.status}: invalid JSON`);
  }
  if (!response.ok || body.status === "error") {
    throw new Error(body.message || `Search proxy HTTP ${response.status}`);
  }
  return body;
}
