...
const { appendStreamTokenToProxyUrl, getProxyAuthHeaders } = require("./api-auth.cjs");
...
// Search v2 requires mobile app headers; Firefox UA returns 400 "invalid token"
const API_V2_SEARCH_HEADERS = {
  ...API_V2_JSON_HEADERS,
  "User-Agent": "okhttp/4.12.0",
  Referer: `https://${MIRROR_HOSTS[0]}/`
};
...
function addProcessedMedia(content, workerOrigin) {
  ...
  if (content && content.downloads) {
    payload.processedSources = content.downloads.map((file) => {
      ...
      const base = `${workerOrigin}/api/download/${encoded}`;
      return {
        id: file.id,
        quality: file.resolution || "Unknown",
        directUrl: file.url,
        proxyUrl: appendStreamTokenToProxyUrl(base, encoded),
        size: file.size,
        format: "mp4"
      };
    });
  ...
  if (content && content.captions && content.captions.length > 0) {
    payload.processedSubtitles = content.captions.map((cap) => {
      const encoded = encodeURIComponent(cap.url);
      const base = `${workerOrigin}/api/subtitles/${encoded}`;
      return {
        id: cap.id,
...
async function postV2Api(url, body, headers = API_V2_JSON_HEADERS) {
...
async function apiViaProxy(proxyOrigin, apiPath, query = {}, options = {}) {
  ...
  const response = await fetch(url.toString(), {
    headers: {
      Accept: "application/json",
      ...getProxyAuthHeaders(options.apiKey)
    }
  });
  let body;
  try {
...
async function searchViaProxy(proxyOrigin, keyword, page, perPage, subjectType, options = {}) {
  const query = { page: String(page), perPage: String(perPage) };
  if (subjectType !== SubjectType.ALL) query.type = String(subjectType);
  return apiViaProxy(proxyOrigin, `/api/search/${encodeURIComponent(keyword)}`, query, options);
}
...
async function getSources(movieId, season, episode, proxyOrigin) {
  ...
  const content = await getOrSetCache(cacheKey, CACHE_TTLS.sources, async () => {
    const sourcesUrl = new URL("https://h5-api.aoneroom.com/wefeed-h5api-bff/subject/download");
    ...
    const data = await makeApiRequest(sourcesUrl.toString(), {
      headers: {
        ...
        "x-client-profile": "web_client_v2.0.4",
        Referer: API_V2_REFERER
      }
    });
